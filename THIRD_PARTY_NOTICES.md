...
- Adapted files: `web/src/components/ui/`, `web/src/hooks/use-mobile.ts`, and the `cn` function in `web/src/lib/cn.ts`.
- Changes: component filename conventions, local import paths, bilingual accessibility labels, and removal of the unused randomized sidebar skeleton. SubLane's application layout is built around the upstream Sidebar components.
