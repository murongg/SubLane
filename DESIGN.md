...
Token budgets open from the member overflow menu in a responsive, vertically scrollable dialog using the existing neutral controls and divided list rows. Keep pool/model scope and the daily or monthly UTC period visible independently of disabled, pending, or exhausted status. Show usage, limits, and settlement amounts in millions of tokens (M), retaining up to six decimal places and explaining the unit beside inputs. Editing preserves pool, model, and period; only the limit and enablement change. Unknown usage stays explicitly pending with an administrator settlement form showing the request ID and known amount. Personal budgets reuse the same list without editing or settlement actions, with visible loading, error, empty, and refresh states.
