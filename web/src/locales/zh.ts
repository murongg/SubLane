...
export const zh: Record<keyof typeof en, string> = {
  ...
  allocationsEmpty: '创建专用账号池并授权成员后，即可添加资源额度。',
  allocationRatioHint:
  ...
  allocationExclusiveHint:
    '请使用专用账号池，并先将其账号从其他所有账号池移除。添加额度后，账号池及池内账号固定；该账号池原有的未绑定 Key 将停止工作。',
  ...
  allocationRatesHint:
    '选择此账号池中的模型后自动填入目录价格，切换模型会更新价格。保存前可以修改；保存后的资源额度版本会固定当时的价格快照。',
  ...
  systemSettings: '系统设置',
  systemSettingsDescription: '管理影响所有工作空间的实例配置。',
  ...
  teamUsageDescription: '按日期、成员、模型和账号池了解团队使用情况。',
  usagePeriod: '统计周期',
  usageToday: '今天',
  ...
  setupWorkspaceDetail:
    '为账号、成员、密钥和用量命名一个独立空间，完成此步骤后才会初始化。',
  startSetup: '开始设置',
  backToWelcome: '返回欢迎页',
  ...
  accountGroups: '账号池',
  groupsDescription: '把订阅账号组成账号池，再按成员授权使用。',
  groupsAssignmentHint:
    '新账号导入后保持未分配。将账号加入账号池，直接授权成员使用，再按需设置额度。',
  ...
  proxiesTitle: '网络代理',
  proxiesDescription: '管理此工作区订阅账号的上游网络出口。',
  proxiesUsageHint:
    '每个账号固定绑定一个代理；更改绑定前，服务端请求会一直使用该出口。',
  addProxy: '添加代理',
  editProxy: '编辑代理',
  deleteProxy: '删除代理',
  saveProxy: '保存代理',
