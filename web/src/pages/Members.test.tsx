...
it('replaces a member’s group grants through the administrator action', async () => {
  const groups = [
    {
      id: 1,
      name: 'Default',
  ...
  const fetchMock = vi
    ...
    .mockImplementation((url: string, init?: RequestInit) => {
      ...
      if (url === '/api/groups/members/2') {
        if (init?.method === 'PUT') {
          granted = JSON.parse(String(init.body)).group_ids
        }
        return Promise.resolve(response({ group_ids: granted }))
      }
      return Promise.resolve(
        response({ members: [syntheticMember], next_cursor: 0 }),
      )
  ...
  await user.click(
    await within(dialog).findByRole('checkbox', { name: 'Default group' }),
  )
  await user.click(
    within(dialog).getByRole('checkbox', { name: 'Project alpha' }),
  )
