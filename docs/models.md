...
Model IDs remain provider-qualified (`codex/…`, `claude/…`, `antigravity/…`), with unqualified Codex compatibility aliases. Explicitly permitted thinking variants use the discovered base model for capability selection while retaining the complete requested ID for permission checks. The UI shows the qualified Codex ID once. Groups are capped at 4,096 distinct qualified models; oversized aggregate catalogs fail explicitly.

Before inference, discovery warms missing/stale catalogs for the requested provider. The scheduler rechecks group policy and account snapshots in its selection transaction, filters out accounts that do not support the requested model, then applies existing concurrency, cooldown and rotation rules. A known unsupported model returns `404 model_not_available`; unknown or over-age capabilities return `503 model_catalog_unavailable` with a retry hint. Other eligible, known accounts may still serve a model when a peer's discovery fails.

Conversation affinity remains unchanged. An existing conversation whose bound account no longer supports the requested model returns `conversation_account_unavailable`; it is never silently moved to another account. No permanent API-key/account binding is added.
