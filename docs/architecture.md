...
| `internal/auth` | Administrator bootstrap, hashes, and sessions | Member keys or upstream credentials |
...
The database runs in WAL mode with foreign keys enabled, a bounded busy timeout, and one open connection. Startup applies ordered embedded SQL migrations and records each migration in the same transaction as its schema change. The initial settings schema is followed by an additive administrator/session migration. No upstream account or member schema exists yet.
...
- Management routes under `/api/` are session-protected by default. Only the four explicit auth endpoints are public; state-changing requests enforce exact origin checks.
...
The root auth gate waits for server state before mounting protected pages. Management queries are enabled only while a user is authenticated, including during redirect transitions. Logout cancels outstanding requests, updates auth state, and removes private query data. Sessions use HttpOnly cookies rather than browser-storage tokens. The complete server contract is documented in [authentication.md](authentication.md).
