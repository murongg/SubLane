...
LEFT JOIN account_groups g ON g.id=b.group_id WHERE b.tenant_id=sqlc.arg(tenant_id) AND b.user_id=sqlc.arg(user_id) ORDER BY b.id;

-- name: SaveTokenBudget :one
INSERT INTO token_budgets(tenant_id,user_id,group_id,model,period,token_limit,enabled,created_at)
VALUES(sqlc.arg(tenant_id),sqlc.arg(user_id),sqlc.arg(group_id),sqlc.arg(model),sqlc.arg(period),sqlc.arg(token_limit),sqlc.arg(enabled),sqlc.arg(created_at)) ON CONFLICT(tenant_id,user_id,group_id,model,period)
...
WHERE e.request_id=sqlc.arg(request_id) AND b.user_id=sqlc.arg(user_id) AND b.tenant_id=sqlc.arg(tenant_id) ORDER BY e.budget_id;
...
AND EXISTS(SELECT 1 FROM token_budgets b WHERE b.id=token_budget_entries.budget_id AND b.tenant_id=sqlc.arg(tenant_id));
...
WHERE b.tenant_id=sqlc.arg(tenant_id) AND b.user_id=sqlc.arg(user_id) AND e.state='pending' GROUP BY e.request_id ORDER BY started_at LIMIT 256;
