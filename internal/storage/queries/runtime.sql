...
SELECT id,sqlc.arg(cooldown_until),sqlc.arg(reason),sqlc.arg(failures),sqlc.arg(last_failure_at),sqlc.arg(revision) FROM accounts WHERE id=sqlc.arg(account_id)
...
UPDATE accounts SET max_concurrency=sqlc.arg(max_concurrency),updated_at=sqlc.arg(now) WHERE id=sqlc.arg(id);
